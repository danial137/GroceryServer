  ...
  async ({ step }) => {
    const { deals, users } = await step.run(
      ...
      async () => {
        ...
        return { deals: product, users: allUser };
      },
    );
...
//Auto-Asign rider after 5 minetus
...
const autoAssignRider = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Wait 5 minutes before assigning a rider
    ...
    const result = await step.run("assign-rider", async () => {
      ...
      // Skip if cancelled or delivered
      ...
      // Find riders that are already busy
